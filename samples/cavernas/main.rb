...
# 3. Copy 'data/world.ldtk' to 'mygame/worlds/world.ldtk'.
# 4. Copy the assets to 'mygame/assets/'.
...
# --- LOGIC: Chaos Tactics Lab ---
class ChaosTacticsLab
  def initialize(args)
    @map_file = "lib/stargateldtk/samples/cavernas/data/world.ldtk"
    @world = nil
    @map = nil
    @adapter = nil
    @player = { x: 5, y: 5 }
  ...
  def render(args)
    ...
    @world.grids.reverse_each do |grid|
      ...
      grid.visual_data.each do |tile|
        args.outputs.sprites << {
          ...
          w: @gs, h: @gs,
          path: "lib/stargateldtk/samples/cavernas/assets/cavernas.png",
          source_x: tile[:src][0],
    ...
    args.outputs.sprites << {
      ...
      w: @gs, h: @gs,
      path: "lib/stargateldtk/samples/cavernas/assets/player/green.png"
    }
...
# --- CORE TICK ---
def tick args
  # Sync with the sample LDtk file
  args.state.sync ||= HotReloadService.new(args, "lib/stargateldtk/samples/cavernas/data/world.ldtk")
